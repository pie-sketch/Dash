...
def generate_status_block(pool_df):
    ...
    for _, row in active_rows.iterrows():
        name = row["Name"]
        load = row["Load"]
        status = "In Progress" if pd.isna(row["End Time"]) else "Complete"
        color = "success" if status == "Complete" else "info"
...
def render_pool_blocks(main_data, seq_data):
    ...
    for _, row in seq_df.iterrows():
        if today_str in row["Pool ID"]:
            current_pool_id = row["Pool ID"]
            break

    past_pool_ids = [row["Pool ID"] for _, row in seq_df.iterrows() if row["Pool ID"] != current_pool_id]

    current = html.Div()
    if current_pool_id:
        current_df = main_df[main_df["Pool ID"] == current_pool_id]
        if not current_df.empty:
            current = generate_status_block(current_df)

    previous = []
    for pid in past_pool_ids:
        pool_df = main_df[main_df["Pool ID"] == pid]
        if not pool_df.empty:
            previous.append(generate_status_block(pool_df))
